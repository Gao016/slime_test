...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        if samples[0].teacher_top_k_ids is not None:
            train_data["teacher_top_k_ids"] = [sample.teacher_top_k_ids for sample in samples]
            train_data["teacher_top_k_log_probs"] = [sample.teacher_top_k_log_probs for sample in samples]

        if samples[0].student_top_k_ids is not None:
            train_data["student_top_k_ids"] = [sample.student_top_k_ids for sample in samples]
            train_data["student_top_k_log_probs"] = [sample.student_top_k_log_probs for sample in samples]

        if samples[0].teacher_on_student_log_probs is not None:
            train_data["teacher_on_student_log_probs"] = [sample.teacher_on_student_log_probs for sample in samples]
